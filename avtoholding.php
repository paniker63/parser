...
ob_start();
	...
	<host>http://avtoxolding.ru/</host>
	<offers>';

for ($i = 1; $i <= 6; $i++) {
	$my_url = 'http://www.avtoxolding.ru/index.php?cstart='.$i.'&do=cat&category=auto';
	$html = file_get_contents($my_url);
	$dom = new DOMDocument();
	@$dom->loadHTML($html);
	$xpath = new DOMXPath($dom);
	
	//$my_xpath_query = "http://www.avtoxolding.ru/index.php?do=cat&category=auto";
	$my_xpath_query = "//div[@class='astitle']/a/@href";
	...
	foreach($result_rows as $result) {
		...
		$id=str_replace("http://www.avtoxolding.ru","", $id);
		...
		$my_xpath_query3 = "//div[@id='dle-content']/h1";
		...
		foreach($result_rows5 as $result5) {
			...
			echo '<image>http://www.avtoxolding.ru'.($result5->textContent).'</image>';
		}
...
// Write final string to file
	file_put_contents('/var/www/parsers/avtoxolding.xml', $htmlStr);
?>