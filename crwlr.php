...
ob_start();
$now1=date("d-m-Y");
$content = file_get_contents("http://crwl.ru/api/rest/v1/get_ads/?api_key=fa638b23fef16a26f2584fbd5d967da3&source=auto.ru&min_date=".$now1."&max_date=".$now1."&region=1");
...
foreach($me as $in){
  ...
  <year>'.$in["year"].'</year>
    ...
    <run>'.$in["run"].'</run>
    <additional-info>
          '.$in["info"].'  Объявление найдено на http://auto.ru/
    </additional-info>
    <state>хорошее</state>
...
file_put_contents('/var/www/parsers/crwlr-autoru.xml', $htmlStr);
?>